...
| 内部函数（不导出） | `ms` + 小驼峰 + `_impl` 后缀或 static | `msMarkRoots_impl()` |
...
```c
static MsObject *fastmath_factorial(MsState *L, int argc, MsObject **argv) {
...
static const MsMethodDef fastmath_methods[] = {
    {"factorial", fastmath_factorial, "factorial(n) -> int"},
    {NULL, NULL, NULL},
};

static const MsModuleDef fastmath_module = {
    "fastmath", "fast math routines", fastmath_methods,
};

MsModuleDef *mslangInit_fastmath(void) { return (MsModuleDef *)&fastmath_module; }
```
