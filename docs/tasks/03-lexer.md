...
交付 mslang 的词法分析器模块（`src/lexer/ms_lexer.h` / `src/lexer/ms_lexer.c`）：把 `.ms` 源文件的 UTF-8 字节流切分为带位置信息（行/列）的 token 流，完整覆盖 `docs/language/01-lexical.md` 的全部词法规则——36 个关键字、整数/浮点/字符串/raw string/f-string/字节串字面量、全部运算符与定界符、注释跳过，以及 Go 同款分号自动插入。词法错误不中断扫描，而是经任务 02 的诊断收集器汇总（单文件上限 20 条）。完成后，任务 04（语法分析器）可以以 `msLexerNext` / `msLexerPeek` 两个调用驱动整个解析过程；本任务自身通过 `tests/c/test_lexer.c` 的 C 单元测试独立验证，不依赖后续任何模块。
...
typedef enum {
  ...
  MS_TOKEN_KW_DEFAULT, MS_TOKEN_KW_DEL, MS_TOKEN_KW_ELSE, MS_TOKEN_KW_EXCEPT,
  MS_TOKEN_KW_FALSE, MS_TOKEN_KW_FINALLY, MS_TOKEN_KW_FOR, MS_TOKEN_KW_FROM,
  MS_TOKEN_KW_FUNC, MS_TOKEN_KW_GLOBAL, MS_TOKEN_KW_IF, MS_TOKEN_KW_IMPORT,
  MS_TOKEN_KW_IN, MS_TOKEN_KW_IS, MS_TOKEN_KW_LAMBDA, MS_TOKEN_KW_NIL,
  MS_TOKEN_KW_NOT, MS_TOKEN_KW_OR, MS_TOKEN_KW_PASS, MS_TOKEN_KW_RAISE,
  MS_TOKEN_KW_RETURN, MS_TOKEN_KW_SELECT, MS_TOKEN_KW_SELF, MS_TOKEN_KW_SUPER,
  MS_TOKEN_KW_TRUE, MS_TOKEN_KW_TRY, MS_TOKEN_KW_WHILE, MS_TOKEN_KW_WITH,

  // 运算符与定界符（01-lexical §6）
  MS_TOKEN_PLUS, MS_TOKEN_MINUS, MS_TOKEN_STAR, MS_TOKEN_SLASH,
  MS_TOKEN_DOUBLE_SLASH, MS_TOKEN_PERCENT, MS_TOKEN_DOUBLE_STAR,
  ...
  MS_TOKEN_STAR_EQUAL, MS_TOKEN_SLASH_EQUAL, MS_TOKEN_DOUBLE_SLASH_EQUAL,
...
- 文件级 `static const` 表：`{const char* name; MsTokenType type;}`，36 项按字典序排列，查找用二分。
...
最长匹配（maximal munch），按前缀长度降序判定：`<<=`/`>>=`/`//=`/`**=`/`...` → `<<`/`>>`/`//`/`**`/`==`/`!=`/`<=`/`>=`/`+=`/`-=`/`*=`/`/=`/`%=`/`&=`/`|=`/`^=`/`:=`/`++`/`--` → 单字符。`.` 的判定优先级：`...` > 浮点（后跟数字） > `MS_TOKEN_DOT`。`!` 后非 `=` 报 E102（语言无逻辑非符号，逻辑非是关键字 `not`）。

### 分号自动插入

与 Go 同规则（01-lexical §7）。状态机：

- 每产出一个真实 token 后更新 `canEndStatement`：`true` 当且仅当 token ∈ {标识符， `MS_TOKEN_INT/FLOAT/STRING/RAW_STRING/BYTES/FSTRING_END`， `MS_TOKEN_KW_TRUE/FALSE/NIL`， `MS_TOKEN_KW_RETURN/BREAK/CONTINUE/PASS/RAISE`， `MS_TOKEN_RIGHT_PAREN/RIGHT_BRACKET/RIGHT_BRACE`， `MS_TOKEN_PLUS_PLUS/MINUS_MINUS`}；其余 token（运算符、`,`、`(`、`[`、`{`、`.`、`:` 等）置 `false`，行尾自然续行。
...
4. 标识符扫描 + 36 关键字二分查找表。验证：全部关键字逐一命中；`chan`、`len` 等内建名产出 `MS_TOKEN_IDENTIFIER`；非 ASCII 标识符。
...
- 标识符与关键字：36 个关键字全量；大小写敏感（`And` 是标识符）；内建函数名（`chan` `len` `print`）是标识符；`_`、`_x9`、含非 ASCII 字符的标识符；数字开头报 E107（按数字非法）或 E102。
...
- 运算符：全表逐一；组合最长匹配 `<<=` vs `<<` vs `<=` vs `<`、`...` vs `.5` vs `.`、`//=` vs `//` vs `/=` vs `/`；裸 `!` 报 E102。
- 分号插入：
...
- [ ] `MsTokenType` 覆盖 01-lexical §4 全部 36 个关键字与 §6 全部运算符/定界符，另有 `++`/`--`（§7 要求）；`msTokenTypeName` 表完整。
