...
- 存储为单个内部 list `_items`，满足堆性质：对任意下标 `i > 0` 有 `_items[(i - 1) // 2] < _items[i]` 或其元素相等（元素间用 `<` 比较，不可比较的元素由 VM 自然抛 `TypeError`，堆自身不做类型校验）。
- `push`：尾部追加后**上浮**——与父节点 `(_i - 1) // 2` 比较，更小则交换，直至根或不再小于父节点。O(log n)。
